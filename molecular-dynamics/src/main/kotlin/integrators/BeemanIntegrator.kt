        ...
        return object : Iterator<SimulationState> {
            ...
            override fun next(): SimulationState {
                ...
                val newR = currentR.indices.map {
                    currentR[it] +
                    currentV[it] * dt +
                            (2.0 / 3.0 * forceUpdater( time, it, currentR, currentV) -
                    1.0 / 6.0 * forceUpdater( time - dt, it, previousR, previousV)) * dt.pow(2)
                }.toDoubleArray()

                val predictedV = currentV.indices.map {
                    currentV[it] +
                    3.0 / 2.0 * forceUpdater(time, it, currentR, currentV) * dt -
                    1.0 / 2.0 * forceUpdater(time - dt, it, previousR, previousV) * dt
                }.toDoubleArray()

                val newV = currentV.indices.map {
                    currentV[it] +
                    1.0 / 3.0 * forceUpdater(time + dt, it, newR, predictedV) * dt +
                    5.0 / 6.0 * forceUpdater(time, it, currentR, currentV) * dt -
                    1.0 / 6.0 * forceUpdater(time - dt, it, previousR, previousV) * dt
                }.toDoubleArray()
